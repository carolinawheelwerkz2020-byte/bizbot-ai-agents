...
const DocsView = lazy(async () => {
  const module = await import('./components/app/DocsView');
  return { default: module.DocsView };
});
...
function summarizeRelayResult(result: unknown) {
  if (!result || typeof result !== 'object') {
    return typeof result === 'string' ? result.slice(0, 500) : '';
  }

  const record = result as Record<string, unknown>;
  if (typeof record.content === 'string' && record.content.trim()) {
    return record.content.trim().slice(0, 700);
  }
  if (typeof record.title === 'string' || typeof record.url === 'string') {
    return [
      record.title ? `Title: ${record.title}` : '',
      record.url ? `URL: ${record.url}` : '',
      record.text ? `Text: ${String(record.text).slice(0, 700)}` : '',
    ].filter(Boolean).join('\n');
  }
  ...
  return JSON.stringify(record, null, 2).slice(0, 700);
}

function createNoTextFallback(agentName: string, lastToolSummary?: string) {
  return [
...
export default function App() {
  ...
  useEffect(() => {
    const loadRunSummaries = async () => {
      const history = await PersistenceService.getRunSummaries();
      setRunSummaries(history);
    };
  ...
  const saveRunSummary = useCallback((summary: RunSummary) => {
  ...
  const handleSendMessage = async (textOverride?: string, agentOverride?: Agent) => {
    ...
    try {
      ...
      let handoffCount = 0;
      let approvalCount = 0;
      let workflowLaunched = false;
      let finalRunAgent = agent;
      let lastToolSummary = '';
      ...
      while (isRunning) {
        ...
        if (response.functionCalls && response.functionCalls.length > 0) {
          ...
          for (const call of response.functionCalls as RelayFunctionCall[]) {
            ...
            try {
              ...
              const summary = summarizeRelayResult(result);
              if (summary) {
                lastToolSummary = `${call.name}: ${summary}`;
              }
            } catch (toolErr: unknown) {
              const relayError = presentError(toolErr, 'relay');
              addLog(`Relay Error: ${relayError.logMessage}`, 'warn');
              lastToolSummary = `${call.name}: ${relayError.summary}`;
              toolResults.push({
                functionResponse: {
                  name: call.name,
        ...
        if (response.text) {
          ...
          const fallbackText = createNoTextFallback(currentAgent.name, lastToolSummary);
          const assistantMsg: Message = {
            role: 'assistant',
          ...
          setMessages(prev => [...prev, assistantMsg]);
          PersistenceService.saveMessage(currentAgent.id, assistantMsg);
          addLog(`Fallback response generated after ${currentAgent.name} returned no text.`, 'warn');
          isRunning = false;
        }
      }
    } catch (err) {
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          <AnimatePresence mode="wait">
            ...
            {activeView === 'workflows' && (
              <Suspense fallback={<ViewLoadingFallback />}>
                <WorkflowsView executeWorkflow={executeWorkflow} onArchitectWorkflow={handleArchitectWorkflow} />
              </Suspense>
            )}
